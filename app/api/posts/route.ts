...
type PostRow = {
  ...
  // when we embed using a foreign key, Supabase returns object OR array depending on schema
  author_profile: Profile | Profile[] | null;
};

function pickProfile(p: PostRow["author_profile"]): Profile | null {
  if (!p) return null;
...
function toApiPost(row: PostRow) {
  const profile = pickProfile(row.author_profile);
  return {
    id: row.id,
    content: row.content,
...
// IMPORTANT: the `!posts_author_id_fkey` part must match YOUR FK name.
// If your FK name differs, see step 1b below.
